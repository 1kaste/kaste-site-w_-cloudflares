...
const DATA_API_BASE_URL = "YOUR_MONGODB_ATLAS_DATA_API_BASE_URL"; // e.g. "https://data.mongodb-api.com/app/data-xxxxx/endpoint/data/v1"
const DATA_API_KEY = "YOUR_MONGODB_ATLAS_DATA_API_KEY";
const DATA_SOURCE = "Cluster0"; // e.g., "Cluster0", check your Atlas settings
const DATABASE_NAME = "kaste-db";
const COLLECTION_NAME = "sitecontents"; // Mongoose default pluralization of 'SiteContent'
...
async function atlasFetch(action, params) {
...
async function handleGetContent() {
    let data = await atlasFetch('findOne', { filter: {} });
    if (!data.document) {
        console.log('No content found, creating default document.');
        await atlasFetch('insertOne', { document: { content: defaultSiteContent } });
        return defaultSiteContent;
    }
    return data.document.content;
}

async function handleUpdateContent(request) {
...
async function handleResetContent() {
    await atlasFetch('deleteOne', { filter: {} });
    await atlasFetch('insertOne', { document: { content: defaultSiteContent } });
    return { success: true };
}
...
async function handleLogin(request, env) {
    ...
    const masterPassword = env.MASTER_PASSWORD || '39344323'; // Fallback for local dev without secrets
            ...
            switch (url.pathname) {
                case '/api/content':
                    if (request.method === 'GET') {
                        responseData = await handleGetContent();
                    } else if (request.method === 'POST') {
                        responseData = await handleUpdateContent(request);
                    }
                    break;
                case '/api/content/reset':
                    if (request.method === 'POST') {
                        responseData = await handleResetContent();
                    }
                    break;
